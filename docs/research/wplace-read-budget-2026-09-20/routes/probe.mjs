...
if (!['mixed', 'tile', 'pixel'].includes(routeMode) || !directory.endsWith('/') || !Array.isArray(rates) || !rates.length || rates.some(rate => !Number.isFinite(rate) || rate <= 0 || rate > 64) || !Number.isFinite(phaseSeconds) || phaseSeconds <= 0 || phaseSeconds > 3600) {
  throw new Error('Use an output directory ending in /, rates in (0,64], and a duration in (0,3600] seconds')
}
if (process.argv.includes('--dry-run')) {
  console.log(JSON.stringify({ rates, phaseSeconds, maxConcurrency, routeMode, retries: 0 }))
  process.exit(0)
}
const rows = []
...
await logTail
const summary = { startedAt, endedAt: new Date().toISOString(), attempted: nextAttempt, routeMode, maxConcurrency, phases, stopped, stopReason: stopped ? 'first unexpected response or transport failure' : 'bounded workload completed without rejection' }
