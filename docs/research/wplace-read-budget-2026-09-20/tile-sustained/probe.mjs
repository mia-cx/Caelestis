...
const maxConcurrency = Number(process.env.WPLACE_PROBE_CONCURRENCY ?? 8)
const routeMode = process.env.WPLACE_PROBE_ROUTE ?? 'mixed'
const tileGridSide = Number(process.env.WPLACE_PROBE_TILE_GRID_SIDE ?? 0)
...
if (!Number.isInteger(maxConcurrency) || maxConcurrency < 1 || maxConcurrency > 16 || !['mixed', 'tile', 'pixel'].includes(routeMode) || !directory.endsWith('/') || !Array.isArray(rates) || !rates.length || rates.some(rate => !Number.isFinite(rate) || rate <= 0 || rate > 64) || !Number.isFinite(phaseSeconds) || phaseSeconds <= 0 || phaseSeconds > 3600) {
...
await logTail
const summary = { startedAt, endedAt: new Date().toISOString(), attempted: nextAttempt, routeMode, maxConcurrency, tilePaths: tiles.length, phases, stopped, stopReason: stopped ? 'first unexpected response or transport failure' : 'bounded workload completed without rejection' }
