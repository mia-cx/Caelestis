...
| Run | Coverage |
| --- | --- |
| Every PR and main push that touches the server | Local Cloudflare Workers; six amd64 Compose combinations per backend runtime; Node/Bun Helm with SQLite/filesystem, CNPG/S3, and MariaDB/S3; adapter contracts; image scans; chart schemas |
| PR or main push without server changes | Nothing. The `changes` job finds no backend, frontend, shared package, deploy, Dockerfile, root workspace, or stack-test file, so every stack job skips and `stack-tests` reports success |
