...
- [x] The release publishes amd64 and arm64 Node, Bun, and frontend images to both registries.
- [x] Both registries receive the default Node, explicit Node, explicit Bun, and frontend manifest lists.
- [x] Release retries verify existing per-architecture images and manifest lists before succeeding.
- [x] GHCR uses the workflow package token; Docker Hub keeps its existing secret.
- [x] Release assets and self-hosting docs expose digest-pinned references for both registries.
- [x] Local release tests, workflow validation, and repository checks pass without publishing a release.
...
- [x] Complete repository checks and file the pull request.
...
- TODO 4: Actionlint and the release helper tests pass. Repository `pnpm lint`, `pnpm check`, `pnpm test`, and `pnpm build` pass with Node 24.20.0. No release or image was published during pull request validation.
