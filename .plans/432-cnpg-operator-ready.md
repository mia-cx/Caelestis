...
## Acceptance criteria
- [ ] `helm (cnpg, node)` and `helm (cnpg, bun)` reach ready on GitHub-hosted runners with a fresh kind cluster. (PR CI run)
...
- [~] Validate with a local kind smoke test and the PR's Portable server CI run. Local smoke done; CI pending on the PR.
