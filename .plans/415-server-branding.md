...
## TODOs
- [ ] Shared contract: `parseHomeCopy`, `parseDiscordInviteUrl`, `ServerInfo` fields in shared and wire-schema, tests.
...
- [ ] Userscript: `Edit server details` action beside rename on the server root row; a `caelestis-server-details` UI element with fields, uploads, clear buttons, and feedback; `state.ts` mutations for patch, upload and delete.
- [ ] Frontend: header logo (image with text fallback), home copy blocks and Discord join action on the home page, social metadata precedence, tests.
