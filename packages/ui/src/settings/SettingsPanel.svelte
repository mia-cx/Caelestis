...
<div class="settings" data-caelestis-scroller>
  ...
  <SettingRow label="Claims only while painting" hint="Hide claims until Wplace's paint drawer is open.">{#snippet children()}<Toggle label="Claims only while painting" checked={model.showPresenceClaimsOnlyWhilePainting} disabled={!model.showPresence || !model.showPresenceClaims} onChange={(value) => emit({ type: 'set-boolean', key: 'showPresenceClaimsOnlyWhilePainting', value })} />{/snippet}</SettingRow>
